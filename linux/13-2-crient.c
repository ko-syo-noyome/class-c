...
//----------------------------------------------------------------------------
    char outdata[1024] = "Hello, network!";
    int outlen = sizeof(outdata);
    // int nsent;
    printf("sending: %.*s\n", outlen, outdata);
    if ((SDLNet_TCP_Send(socket, outdata, outlen)) < outlen) error(TCP_Send);
    //printf("sending: %s\n",outdata);
//----------------------------------------------------------------------------

    char indata[1024];
    int inlen=SDLNet_TCP_Recv(socket,indata,sizeof(indata));
        if (outlen==0)
            warning(TCP_Recv);
    ...
    // char indata[1024];
    // int inlen;
    // if ((inlen = SDLNet_TCP_Recv(socket, indata, sizeof(indata))) == 0)
    //     warning(TCP_Recv);
    // else printf("received: %.*s\n", inlen,indata);
